...
- The main method (in the ImageProcessingProgram) was updated to support the updated command line
  arguments
...
- The pixel class has been changed to allow support for alpha values (transparency). Alpha values
  are not used in any image calculations/modifications, they are merely preserved when
  saving/loading
  whenever possible.
...
1. Features interface
    1. The features interface is used to represent high level features/abilities that we can
       perform such as saving/loading files and executing image operations. The Features interface
       and
...
2. ImageProcessingModelState interface
    1. This interface is one part of the model design. The ImageProcessingModelState interface
       is the "parent" to the ImageProcessingView interface. Thus, the method headers in this
       interface
...
This design promotes much better organization for loading/saving different file types. It also
allows
for near unlimited flexibility when it comes to adding support for new file types. You only have to
change very
...
To run the program in GUI mode, run either the jar file or the main method in the
...
The image that is currently being shown will be scaled if it is too small. Specifically, the image
will be enlarged so that it's width is
...
For the specific syntax of each command and more information, type "menu". imageName is the name of
the image
